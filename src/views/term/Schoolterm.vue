    ...
    <form-drawer ref="formDrawerRef" :title="drawerTitle" size="45%" destroyOnClose @submit="handleDrawerSubmit" >
        ...
        class="demo-ruleForm"   :size="formSize" status-icon>
            <el-form-item label="学期名称" prop="termname">
                <el-input v-model="formModel.termname" />
            </el-form-item>
            <el-form-item label="描述" prop="termdesc">
                <el-input v-model="formModel.termdesc" type="textarea" />
            </el-form-item>
        </el-form>
    </form-drawer>
</template>
...
import { RequestListData, RequestSaveData, RequestInfoData, RequestUpdateData, RequestDeleteData } from '~/api/term.js'
...
const handleEdit = (row) => {
    ...
    RequestInfoData(row.id).then(res=>{
        if(res.code == 200){
            ...
            formModel.termname = res.data.termname
            formModel.termdesc = res.data.termdesc
        }
    })
